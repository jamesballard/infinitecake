 ...
 */
class FactUserVerbRuleDate extends AppModel {
 ...
 */
	public $useTable = 'fact_user_verb_rule_date';
            ...
            if($date->format("d-M") != '29-Feb') {
                ...
                foreach ($rule_conditions[0]['Condition'] as $rule_condition) {
                    ...
                    $value = $this->find('first', array(
                            ...
                            'recursive' => 0, //int
                            'fields' => "SUM(FactUserVerbRuleDate.total) as total", //array of field names
                        )
                    );
                    if($value[0]['total']) {
                        $count = $value[0]['total'];
